...
def map_interface():
    ...
    return {
        ...
        'connection_color': st.sidebar.color_picker("Connection Line", "#FFE66D"),
        'show_connections': st.sidebar.checkbox("Show Connections", True),
        'use_clustering': st.sidebar.checkbox("Cluster Markers", True),
        'show_heatmap': st.sidebar.checkbox("Show Heatmap", False)
    }

def main(origins_df, destinations_df):
    st.title("Commute Map")
    opts = map_interface()
    commute_map = create_commute_map(origins_df, destinations_df, opts)

    st.components.v1.html(commute_map._repr_html_(), height=600)
    return commute_map
